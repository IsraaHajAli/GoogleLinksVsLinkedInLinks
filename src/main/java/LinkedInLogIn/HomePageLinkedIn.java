...
public class HomePageLinkedIn extends BasePage {
    ...
    public void searchAtLinkedIn(String searchData, String searchType) {
         ...
         * **/
        List<String> [] resultData = new List[1];
       ...
       /** resultData[1] = new ArrayList<>();
        ...
        List<WebElement> results = locators.LinkedInResults;


        for (int i = 0; i < Math.min(5, results.size()); i++) {
            // below is a javaScript extractor
           /** JavascriptExecutor js = (JavascriptExecutor) driver;
            resultData[0].add((String) js.executeScript("return arguments[0].childNodes[0].textContent;", locators.names.get(i)));
            resultData[1].add(locators.resultPosition.get(i).getText());
            resultData[2].add(locators.locations.get(i).getText());
            **/

            resultData[0].add(locators.profilesLinks.get(i).getAttribute("href"));
